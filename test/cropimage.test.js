...
( function( $ ){
	...
	$.fn != undefined ? $.fn.cropimage = core : console.log('JQuery 2.0.0 undefined')
					...
					Cropper( originDetails, ADAPTED, function( CROPPED ){
							...
							.touchstart( function(e){
									...
									if( !NO_MOVE ){
										...
										MOVING.t = $_CROPPER
										MOVING.x = e.originalEvent.touches[0].pageX - $_CROPPER.position().left
										MOVING.y = e.originalEvent.touches[0].pageY - $_CROPPER.position().top
									}
							...
							$_TRIGGERS.mousedown( function(e){
									...
									RESIZING.x = e.pageX - $_CROPPER.position().left
							...
							function moving( e, MOVING, touch ){
								...
								var LEFT = ( touch ? e.originalEvent.touches[0].pageX : e.pageX ) - MOVING.x,
										TOP = ( touch ? e.originalEvent.touches[0].pageY : e.pageY ) - MOVING.y,
							...
							function resizing( e, RESIZING, touch ){
								...
								// image relative position
										LEFT = ( touch ? e.originalEvent.touches[0].pageX : e.pageX ) - $_ADAPTER.offset().left,
										TOP = ( touch ? e.originalEvent.touches[0].pageY : e.pageY ) - $_ADAPTER.offset().top,
								...
								$('#move-stats').html( '<h3>Resizing</h3>'
																		+'<small>zoom: '+ zoom +' >> Max: '+ OPTIONS.zoomMax +'</small><br>'
																		+'<small>NO_MOVE: '+ NO_MOVE +'</small><br><br>'
																		+'<small>Crop type: '+ RESIZING.t.data('action') +'</small><br>' 
																		...
																		+'<small>CropLimitLeft: ( '+ ADAPTED.left +', '+ CropLimitLeft +' )</small><br>' 
																		+'<small>CropLimitTop: ( '+ ADAPTED.top +', '+ CropLimitTop +' )</small><br>'
																		+'<small>CropLimitRight: ( '+ ADAPTED.left +', '+ CropLimitRight +' )</small><br>' 
																		+'<small>CropLimitBottom: ( '+ ADAPTED.top +', '+ CropLimitBottom +' )</small><br>'
																	)
